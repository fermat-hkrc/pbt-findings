...
- **Precision:** **85.7%** (102 FIXED / 119 decided)
- **False-positive rate:** **14.3%** (17 NON-ISSUE)
...
- Non-issue write-ups: `~/cloned/*/pbt-out/bug_reports/non-issue/` (**17** DTS-stamped)
...
| FIXED | 102 | 85.7% |
| NON-ISSUE | 17 | 14.3% |
| **Total decided** | **119** | 100% |

- **Precision:** **102/119 = 85.7%** — just under nine in ten closed tickets were real bugs.
- **False-positive rate:** **17/119 = 14.3%**.
...
All **17** maintainer-rejected tickets. Useful as negative examples for future filing.
...
| Caller-owned or split-API contract | 4 | Read in-tree callers and sibling APIs before claiming inconsistency |
...
Mixed outcomes: maintainers accepted **8** and rejected **2**. Net precision **80%**.
...
| NON-ISSUE | 17 | Maintainer-rejected (cloned inventory) |
...
1. **Overall precision is high (85.7%)** — PBT filings that reach a DTS decision are usually real defects.
2. **False positives cluster in a few patterns** (17 tickets): by-design helpers, dead/unreachable / dropped-from-trunk code, caller-owned / split-API contracts, shipped CAPI / product omissions, flag-dependent crashes — not flaky reproduction.
